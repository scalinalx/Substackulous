  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-amber-300 via-amber-400 to-[#FFF8DC] pt-20">
      ...
      <div className="container mx-auto px-4 flex flex-col items-center text-center pt-20 relative z-10">
        <h1 className="text-4xl md:text-6xl font-bold text-amber-800 max-w-4xl mb-6 leading-tight">
          Join 40,000+ Successful Substack Creators — Grow Smarter, Convert Faster
        </h1>
        <p className="text-xl md:text-2xl text-amber-800/80 max-w-2xl mb-8">
          Substackulous gives you AI-powered tools to supercharge your content,
          audience growth, and revenue
        </p>
        <Link 
          href="/dashboard"
          className="group bg-amber-800 hover:bg-amber-700 text-white px-8 py-4 rounded-full font-medium text-lg transition-all duration-300 flex items-center space-x-2 mb-12 shadow-lg hover:shadow-xl"
        >
          <span>Start Free Trial (Boost Subscribers in 30 Days!)</span>
          <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
        </Link>
        <div className="flex flex-col items-center">
          <p className="text-amber-800/60 mb-4">Trusted by the best of Substack</p>
          ...
          <p className="text-amber-800/60 text-lg">+40k more...</p>
        </div>
      </div>
    </div>
  );
